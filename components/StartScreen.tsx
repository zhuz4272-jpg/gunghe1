...
export const StartScreen: React.FC<StartScreenProps> = ({ onGenerate, isGenerating }) => {
  return (
    <div className="bg-gradient-to-br from-[#ffe8cc] via-[#ebf7ee] to-[#d6f5d6] h-full w-full relative overflow-hidden flex flex-col items-center animate-gradient-xy text-slate-800">
      ...
      {/* Main Container with Scroll Support for small screens */}
      <div className="w-full h-full overflow-y-auto overflow-x-hidden relative z-10">
        <div className="w-full max-w-md mx-auto min-h-full flex flex-col px-6 py-6 justify-between">
          ...
          {/* Main Content */}
          <main className="flex-1 flex flex-col items-center justify-center w-full relative py-8">
            ...
            {/* Seed Container */}
            <div className={`relative w-full aspect-[4/5] max-h-[45vh] sm:max-h-[500px] flex items-center justify-center ${isGenerating ? 'animate-pulse scale-95 transition-all duration-700' : 'animate-float'}`}>
              ...
              <div className="relative h-full aspect-square rounded-[3rem] overflow-hidden shadow-2xl shadow-green-900/10 border border-white/40 backdrop-blur-sm bg-gradient-to-br from-white/30 to-white/10 z-10">
                <img 
            ...
            {/* Text Content */}
            <div className={`text-center mt-6 space-y-2 z-20 transition-opacity duration-500 ${isGenerating ? 'opacity-50' : 'opacity-100'}`}>
