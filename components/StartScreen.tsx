...
export const StartScreen: React.FC<StartScreenProps> = ({ onGenerate, isGenerating }) => {
  return (
    <div className="bg-gradient-to-br from-[#ffe8cc] via-[#ebf7ee] to-[#d6f5d6] h-full w-full relative overflow-hidden flex flex-col items-center animate-gradient-xy text-slate-800">
      ...
      <div className="w-full max-w-md mx-auto h-full flex flex-col relative z-10 px-6 pt-6 pb-6 justify-between">
        
        {/* Header */}
        <header className="flex justify-between items-center py-2 w-full shrink-0">
        ...
        <main className="flex-1 flex flex-col items-center justify-center w-full relative min-h-0">
          ...
          <div className="absolute top-0 left-1/2 -translate-x-1/2 w-[2px] h-40 bg-gradient-to-b from-transparent via-white/80 to-transparent blur-[2px] opacity-70"></div>

          {/* Seed Container */}
          <div className={`relative w-full max-h-[50vh] aspect-[4/5] flex items-center justify-center ${isGenerating ? 'animate-pulse scale-95 transition-all duration-700' : 'animate-float'}`}>
            <div className="absolute inset-0 m-auto w-64 h-64 bg-gradient-to-tr from-orange-100 to-green-100 rounded-full blur-3xl opacity-80"></div>
            
            <div className="relative w-64 h-64 sm:w-72 sm:h-72 rounded-[3rem] overflow-hidden shadow-2xl shadow-green-900/10 border border-white/40 backdrop-blur-sm bg-gradient-to-br from-white/30 to-white/10 z-10">
              <img 
          ...
          {/* Text Content */}
          <div className={`text-center mt-4 mb-2 space-y-2 z-20 transition-opacity duration-500 shrink-0 ${isGenerating ? 'opacity-50' : 'opacity-100'}`}>
            ...
            <div className="relative flex items-center justify-between px-8 h-full">
              <div className="flex items-center space-x-4">
                ...
                <div className="flex flex-col items-start text-white">
                  <span className="text-xl font-bold tracking-wider text-shadow-sm">
